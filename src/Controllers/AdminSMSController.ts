        ...
        try {
            ...
            if (data.token && data.phone) {
                ...
                if (tok) {
                    ...
                    const tty: GetTTY = await Fetch.request('http://localhost:3020/getalltty', {token: token});
                    ...
                    if (tty.status) {
                        ...
                        for (const i of tty.data) {
                            ...
                            if (i.phone === data.phone) {
                                ...
                                const getSmsData: GetSMSData = await Fetch.request('http://localhost:3020/getallmessages', {token: token, port: i.tty});
